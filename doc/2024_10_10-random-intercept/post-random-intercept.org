...
Considering the solution \(\sigma^2=\frac{1}{np}\sum_{i=1}^n\sum_{j=1}^p (Y_{i,j}-\mu_{i,j})^2=\frac{1}{p}\sum_{j=1}^p \widehat{\sigma}^2_j\) we need to have:
#+BEGIN_EXPORT latex
\begin{align*}
\frac{1}{p(p-1)/2}\sum_{j < j^{\prime}}\frac{\widehat{\sigma}_j \widehat{\sigma}_{j^{\prime}}}{\sigma^2}\widehat{\rho}_{j,j^{\prime}} &= \rho
...
#+BEGIN_SRC R :exports none :results output :session *R* :cache no
sigma2 - sigma2/(1-rho) + (1/p)*rho/((1-rho)^2+rho*(1-rho)*p) * sum(M.resVcov)
#+END_SRC
...
#+BEGIN_SRC R :exports none :results output :session *R* :cache no
1/(1-rho) - rho/p * (p+p*(p-1)*rho)/((1-rho)^2+rho*(1-rho)*p)
1/(1-rho) - (rho + (p-1)*rho^2)/((1-rho)^2+rho*(1-rho)*p)
(1-rho + rho*p - rho - (p-1)*rho^2)/((1-rho)^2+rho*(1-rho)*p)
#+END_SRC
