...
\label{sec:org316544d}

\subsection{Notations}
\label{sec:orgbc39ee7}
...
\label{sec:org7ddeec4}

\subsubsection{Theory}
\label{sec:org0fa2a83}

\subsubsection{Numerical example}
\label{sec:orgc954a9e}
...
&= \rho + \rho \left(\frac{1}{p}\sum_{j=1}^p \frac{\widehat{\sigma}^2_j}{\sigma^2} - 1\right) \frac{\rho(p-2)  + 2}{\rho^2(p-1) + 1}
\end{align*}

\subsection{Variance parameter (ML)}
\label{sec:org8057e5a}
...
Using that \(\sigma^2=\widehat{\sigma}^2=\frac{1}{p}\sum_{j=1}^p \widehat{\sigma}^2_j\) we obtain:
\begin{align*}
1 =& \frac{1}{(1- \rho)}  - \frac{1}{p}\frac{\rho}{(1-\rho)^2+\rho(1-\rho)p} \left(p + p(p-1)\rho\right) \\
  =& \frac{1}{(1- \rho)}  - \frac{\rho + (p-1)\rho^2}{(1-\rho)^2+\rho(1-\rho)p}  \\
  =& \frac{1-\rho + \rho p  - \rho - (p-1)\rho^2}{(1-\rho)^2+\rho(1-\rho)p} \\
  =& \frac{1-2\rho+\rho^2 + \rho (1 - \rho) p}{(1-\rho)^2+\rho(1-\rho)p} = 1
\end{align*}
which is true. Thus the proposed solution satisfies the score equations.
...
\begin{align*}
0 =& \frac{1-\rho}{\rho^2(p-1) + 1} \left(\frace{\widehat{\sigma}^2}{\sigma^2}-1\right) \\
\end{align*}
