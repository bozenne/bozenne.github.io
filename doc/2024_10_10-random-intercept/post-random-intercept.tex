...
\label{sec:orgaf6adbd}

\subsection{Theory}
\label{sec:org6bfc4e0}
...
=&  \frac{\widehat{\sigma}^2}{p^2} \left(p + 2\sum_{j < j^{\prime}}\widehat{\rho}_{j,j^{\prime}}\right) 
\end{align*}

we obtain:
\begin{align*}
\frac{\sigma^2}{\widehat{\sigma}^2} =& \frac{1}{(1- \rho)}  - \frac{1}{p}\frac{\rho}{(1-\rho)^2+\rho(1-\rho)p} \left(p + 2\sum_{j < j^{\prime}}\widehat{\rho}_{j,j^{\prime}}\right)
\end{align*}
Using the score equation w.r.t to the correlation parameter:
\begin{align*}
(1 - \rho)\frac{\sigma^2}{\widehat{\sigma}^2} =& 1 - \frac{1}{p}\frac{\rho}{1-\rho+\rho p} \left(p + \rho p (p-1) + \rho p (p-1) \left(1- \frac{\widehat{\sigma}^2}{\sigma^2}\right) \frac{\rho(p-2)  + 2}{\rho^2(p-1) + 1}\right) \\
