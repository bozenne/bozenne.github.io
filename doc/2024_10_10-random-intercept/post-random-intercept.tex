...
\label{sec:orgb91703e}

\subsection{Notations}
\label{sec:org3c1c31a}
...
\label{sec:orgd9b2f70}

\subsubsection{Theory}
\label{sec:org188e280}

\subsubsection{Numerical example}
\label{sec:org7e64d65}
...
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
dfL$res.norm <- dfL$res/sqrt(mean(dfL$res^2))
dfWres.norm <- reshape(dfL[,c("id","visit","res.norm")],
                  direction = "wide", idvar = "id", timevar = "visit")
M.MLcor <- crossprod(as.matrix(dfWres.norm[,-1]))/NROW(dfWres.norm)
...
Using that \(\sigma^2=\frac{1}{np}\sum_{i=1}^n\sum_{j=1}^p (Y_{i,j}-\mu_{i,j})^2=\frac{1}{p}\sum_{j=1}^p \widehat{\sigma}^2_j\) we obtain:
\begin{align*}
\frac{1}{p(p-1)/2}\sum_{j < j^{\prime}}\frac{\widehat{\sigma}_j \widehat{\sigma}_{j^{\prime}}}{\sigma^2}\widehat{\rho}_{j,j^{\prime}} &= \rho
...
\end{align*}
where \(\widehat{\sigma}^2=\frac{1}{np}\sum_{i=1}^n \sum_{j=1}^p
\left(Y_{i,j}-\mu_{i,j}\right)^2\). Denoting
\(\mu_{i,.}=\frac{1}{p}\sum_{j=1}^p Y_{i,j}\) and \(\mu_{.,.}=\alpha +
\frac{p_1}{p} \beta\), we can re-use ANOVA-style decomposition for the
sum of squares:
\begin{align*}
\widehat{\sigma}^2 = \frac{1}{np} \sum_{i=1}^n \sum_{j=1}^p \left(Y_{i,j} - \mu_{.,.} \right)^2
=& \frac{1}{np} \sum_{i=1}^n \sum_{j=1}^p \left(Y_{i,j} - \mu_{i,.}\right)^2 + \frac{1}{np} \sum_{i=1}^n \sum_{j=1}^p \left(\mu_{i,.} - \mu_{.,.}\right)^2 \\
& + \frac{2}{np} \sum_{i=1}^n \sum_{j=1}^p \left(Y_{i,j} - \mu_{i,.}\right)\left(\mu_{i,.} - \mu_{.,.}\right)
\end{align*}
The later terms is 0 because \(\sum_{j=1}^p Y_{i,j} - \mu_{i,.} = p
\mu_{i,.} - p\mu_{i,.} =0\). So:
\begin{align*}
\frac{1}{np}\sum_{i=1}^n \sum_{t=1}^t \left(Y_{it}-Y_{i.}\right)^2  =& \widehat{\sigma}^2 - \frac{1}{n} \sum_{i=1}^n \left(\mu_{i,.} - \mu_{.,.}\right)^2 \\
=& \widehat{\sigma}^2 - \frac{1}{np^2} \sum_{i=1}^n \sum_{j=1}^p \sum_{j^{\prime}=1}^p \left(Y_{i,j}-\mu_j\right)\left(Y_{i,j^{\prime}}-\mu_{j^{\prime}}\right) \\
=& \widehat{\sigma}^2 - \frac{1}{p^2} \left(\sum_{j=1}^p \widehat{\sigma}^2_j + 2\sum_{j < j^{\prime}}\widehat{\sigma}_j \widehat{\sigma}_{j^{\prime}}\widehat{\rho}_{j,j^{\prime}}\right) 
\end{align*}
Therefore:
\begin{align*}
\sigma^2 =& \frac{\widehat{\sigma}^2}{(1- \rho)}  - \frac{\rho p^2}{(1-\rho)^2+\rho(1-\rho)p} \left(\widehat{\sigma}^2 + \frac{1}{p^2} \left(\sum_{j=1}^p \widehat{\sigma}^2_j + 2\sum_{j < j^{\prime}}\widehat{\sigma}_j \widehat{\sigma}_{j^{\prime}}\widehat{\rho}_{j,j^{\prime}}\right) \right)
%1- \rho  =& \widehat{\sigma}^2/\sigma^2 \left(1  - \frac{\rho}{1-\rho+\rho} \left(\widehat{\sigma}^2 + \frac{1}{p^2} \left(\sum_{j=1}^p \widehat{\sigma}^2_j + 2\sum_{j < j^{\prime}}\widehat{\sigma}_j \widehat{\sigma}_{j^{\prime}}\widehat{\rho}_{j,j^{\prime}}\right) \right)
\end{align*}
...
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
dfRes <- reshape(dfL[,c("id","visit","res")],
                 direction = "wide", idvar = "id", timevar = "visit")
mean(crossprod(as.matrix(dfRes[,-1])))/n
\end{lstlisting}
