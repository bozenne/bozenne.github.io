...
\label{sec:orgfffed9d}
\subsection{Set the working directory}
\label{sec:org8963e37}
...
\label{sec:orgb9c74e7}
\subsection{Categorize age into groups}
\label{sec:org38af6a5}
\begin{lstlisting}[language=r,numbers=none]
...
\label{sec:orgdaf0c66}
\subsection{Introduction}
\label{sec:org50da00a}
...
\label{sec:orgcb0ad28}
\subsubsection{Number of rows and columns}
\label{sec:org203399f}
\begin{lstlisting}[language=r,numbers=none]
dim(dtW.data)
\end{lstlisting}
...
\label{sec:org44a6aee}
\subsubsection{Univariate melt}
\label{sec:org1a81a18}
...
\label{sec:orga4ea883}

\subsubsection{Univariate}
\label{sec:org560a919}
Data in the long format:
\begin{lstlisting}[language=r,numbers=none]
head(dtL.data)
\end{lstlisting}
...
\label{sec:org31530f9}
\subsection{Descriptive plots}
\label{sec:org6b312de}
...
\label{sec:orga502c4b}

\subsubsection{Greek letter in facet}
\label{sec:org43c5ddc}
\begin{lstlisting}[language=r,numbers=none]
seqX <- 1:15
...
\end{center}
\subsection{Font}
\label{sec:org816b21d}
\subsubsection{Display available fonts}
\label{sec:org711ed81}
\begin{lstlisting}[language=r,numbers=none]
windowsFonts()
\end{lstlisting}
...
\end{verbatim}
\section{Modeling}
\label{sec:orgd67b838}
\subsection{Test proportions}
\label{sec:orgfbd1a4e}
\begin{lstlisting}[language=r,numbers=none]
...
\label{sec:org4b16ee3}
\subsubsection{Data}
\label{sec:org2bcd6ac}
...
\label{sec:orgb311c04}
\subsection{Apply with progress bar}
\label{sec:org4d324c0}
...
\label{sec:orgc99e38b}
\subsubsection{Detect the number of cores}
\label{sec:orgc6ded66}
...
\end{verbatim}
\subsection{Interaction between latent variables}
\label{sec:org3a49b6e}

Simulate LVM with interaction:
\begin{lstlisting}[language=r,numbers=none]
mSim <- lvm(x1+x2+x3 ~ etaX,
            y1+y2+y3 ~ etaY,
            z ~ etaXY)
covariance(mSim, etaX~etaY) <- 0.25
transform(mSim, etaXY ~ etaX+etaY) <- function(x){x[1]*x[2]+rnorm(NROW(x))}
dSim.GS <- sim(mSim, n=1e3, seed=42) # Default is all parameters are 1
\end{lstlisting}

Oracle regression:
\begin{lstlisting}[language=r,numbers=none]
summary(lm(z ~ etaX*etaY, data = dSim.GS))$coef
\end{lstlisting}
...
\begin{verbatim}
                Estimate Std. Error    t value     Pr(>|t|)
(Intercept)  0.008112885 0.04631895  0.1751526 8.609953e-01
etaX         0.036613618 0.04705011  0.7781835 4.366457e-01
etaY        -0.104524672 0.04634788 -2.2552204 2.433591e-02
etaX:etaY    1.042602480 0.04422953 23.5725438 5.504357e-98
\end{verbatim}


Observable dataset:
\begin{lstlisting}[language=r,numbers=none]
dSim.obs <- dSim.GS[,c(paste0("x",1:3),paste0("y",1:3),"z")]
\end{lstlisting}

Latent variable model with interaction on the observable data:
\begin{lstlisting}[language=r,numbers=none]
## linear LVM estimable on the observed data
m1 <- lvm(x1+x2+x3 ~ etaX,
          y1+y2+y3 ~ etaY,
          latent=~etaX+etaY)
covariance(m1) <- etaX~etaY
## linear LVM if m1 is known otherwise non-linear
m2 <- lvm(z ~ etaX + etaY + etaX:etaY)
mypred <- function(mu,var,data,...) {
  ## E[f(etaX,etaY)] = E[\alpha etaX + \beta etaY + \gamma etaX etaY | X,Y]
  ##                 = \alpha E[etaX|X,Y] + \beta E[etaY|X,Y] + \gamma E[etaX etaY|X,Y]
  ##                 = \alpha E[etaX|X,Y] + \beta E[etaY|X,Y] + \gamma (E[etaX|X,Y]E[etaY|X,Y] + Cov(etaX,etaY|X,Y)
  ## NOTE: var["etaX","etaY"|X,Y]=0 without covariance link
  ##       var["etaX",G|X,Y]=0 when G is one of the observed covariates, i.e., is in X
  cbind("etaX" = mu[,"etaX"],
        "etaY" = mu[,"etaY"],
        "etaX:etaY" = mu[,"etaX"] * mu[,"etaY"] + var["etaX","etaY"])
}
ee2 <- twostage(m1, model2=m2, data=dSim.obs, predict.fun=mypred)
summary(ee2)
\end{lstlisting}
...
Regressions:                                                      
   z~etaX            0.06320    0.07094  0.89084    0.373  0.03031
   z~etaY           -0.08678    0.06607 -1.31344    0.189 -0.04557
   z~etaX:etaY       1.03636    0.09035 11.47117   <1e-12  0.46523
Intercepts:                                                       
   z                 0.01752    0.05555  0.31545   0.7524  0.00985
Residual Variances:                                               
   z                 2.47678    0.11838 20.92265           0.78271
───────────────────────────────────────────────────────────────────────────────────────────────────────────
Estimator: generic 
───────────────────────────────────────────────────────────────────────────────────────────────────────────

 Number of observations = 1000 
 Log-Likelihood = -1872.418 
 BIC = 3779.375 
 AIC = 3754.836 
───────────────────────────────────────────────────────────────────────────────────────────────────────────
R-squared 
...
In the lava package \href{https://cran.r-project.org/web/packages/lava/vignettes/nonlinear.html}{vignette} about interactions, section
'Specification of general functional forms', the example is given for
an quadratic interaction, i.e. \(\beta_1 \eta + \beta_2 \eta^2 +
\gamma G + \delta_1 \eta G + \delta_2 \eta G\). Here
\(\Esp[\eta^2|X,Y]=\Esp[\eta|X,Y]^2 + \Var[\eta|X,Y]\).
\subsection{Handling left, right, and interval censored data}
\label{sec:orgf5c94ed}
...
\label{sec:org636dda1}
\subsection{Profile code R}
\label{sec:org26ec4af}
