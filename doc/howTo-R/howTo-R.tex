...
\label{sec:org9bde33c}
\subsection{Set the working directory}
\label{sec:orgb8a4184}
...
\label{sec:orga503f91}
\subsection{Categorize age into groups}
\label{sec:orgd6b7d20}
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\label{sec:orgaea7a2d}
\subsection{Introduction}
\label{sec:orga0244f2}
...
\label{sec:org78e2462}
\subsubsection{Number of rows and columns}
\label{sec:orgd9b740b}
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
dim(dtW.data)
\end{lstlisting}
...
\label{sec:org27ac498}
\subsubsection{Univariate melt}
\label{sec:org4bae2af}
...
\label{sec:orge8fc572}

\subsubsection{Univariate}
\label{sec:org307e5cf}
Data in the long format:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
head(dtL.data)
...
\label{sec:org358213f}
\subsection{Descriptive plots}
\label{sec:org10b3796}
...
\label{sec:org4aba91d}
\subsection{Path diagram}
\label{sec:org707fa48}
Using lava:
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\section{Modeling}
\label{sec:org8cd6720}
\subsection{Test proportions (from Paul Blanche)}
\label{sec:org4f8b237}
\lstset{language=r,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
library(exact2x2)
library(Publish)
...
rownames(AllRes) <- c("Risk differrence","Relative risk","Odds ratio","p-value")
AllRes
\end{lstlisting}
...
AllRes[3,1] <- paste0(specdec(xEuF$estimate,k=3)," (",specdec(xEuF$conf.int[1],k=3),";",specdec(xEuF$conf.int[2],k=3),")")
\end{lstlisting}
...
AllRes[3,3] <- paste0(specdec(xEcF$estimate,k=3)," (",specdec(xEcF$conf.int[1],k=3),";",specdec(xEcF$conf.int[2],k=3),")")

ciRDEcF <- binomMeld.test(x1=tab[2,1],n1=sum(tab[2,]),
						  x2=tab[1,1],n2=sum(tab[1,]),
						  parmtype="difference")
AllRes[1,3] <- paste0(specdec(ciRDEcF$estimate,k=3)," (",specdec(ciRDEcF$conf.int[1],k=3),";",specdec(ciRDEcF$conf.int[2],k=3),")")

ciRREcF <- binomMeld.test(x1=tab[2,1],n1=sum(tab[2,]),
						  x2=tab[1,1],n2=sum(tab[1,]),
						  parmtype="ratio")
AllRes[2,3] <- paste0(specdec(ciRREcF$estimate,k=3)," (",specdec(ciRREcF$conf.int[1],k=3),";",specdec(ciRREcF$conf.int[2],k=3),")")
\end{lstlisting}
...
AllRes[3,2] <- paste0(specdec(xA$or,k=3)," (",specdec(xA$or.lower,k=3),";",specdec(xA$or.upper,k=3),")")
AllRes[2,2] <- paste0(specdec(xA$rr,k=3)," (",specdec(xA$rr.lower,k=3),";",specdec(xA$rr.upper,k=3),")")
AllRes[1,2] <- paste0(specdec(xA$rd,k=3)," (",specdec(xA$rd.lower,k=3),";",specdec(xA$rd.upper,k=3),")")
...
\begin{verbatim}
                 Exact: usual Fisher   Asymptotic               Exact: central Fisher  
Risk differrence ""                    "-0.433 (-0.774;-0.092)" "-0.433 (-0.748;0.017)"
...
\label{sec:org0c19269}
\subsection{Apply with progress bar}
\label{sec:org341d990}
...
\label{sec:org255aca5}
\subsubsection{Detect the number of cores}
\label{sec:org9a2b393}
...
\label{sec:orgde9d53a}
\subsection{Profile code}
\label{sec:org7772efb}
