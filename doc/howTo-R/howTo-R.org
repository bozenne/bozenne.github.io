...
#+BEGIN_SRC R :results graphics :file "./figures/fig-hist.pdf" :exports results :session *R* :cache yes
print(gg.hist)
#+END_SRC
...
* Modeling
** Test proportions (from Paul Blanche)
#+BEGIN_SRC R :exports both :results output :session *R* :cache no
library(exact2x2)
library(Publish)
...
# OLD VERSION
#+BEGIN_SRC R :exports none :results output :session *R* :cache yes
filename <- tempfile()
