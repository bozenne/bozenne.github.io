...
Here are some personal thoughs about how to report the statistical
...
mentionned further. A comprehensive description of the statistical
...
- Model parameters were estimated by Maximum Likelihood Estimation (MLE) and uncertainty was derived from on asymptotic likelihood theory.
...
testing procedure is more a technicality, i.e., does not matter too
much for interpreting the results (and can thus be omitted). An
exception is when performing multiple tests where it is important for
the interpretation to report whether any adjustment was performed. 
...
It common to see in applied par
...
the authors, just mentionning the version of the software/packages is
...
``` to obtain how to cite the R software and related packages.

## Common mistakes

- **reporting p-values in a descriptive table** (typically) table 1,
    or, more generally, systematic comparison of patient
    characteristics: "we compared the clinical characteristics between
    treatment groups using t-test and chi-squared test". It is
    difficult to understand such analyses and they are against current
    guidelines (e.g. [STROBE](https://doi.org/10.1371/journal.pmed.0040297), section 14.a).

- **claiming that all model assumptions were check are met**. This is
...
- **concluding about treatment effect in a single arm study**. In a
    study where individuals, unmedicated at baseline, are all given
    the same treatment and follow over time one cannot disentangle the
    treatment effect from time effects (regression to the mean,
    seasonal effect, disease progression,...).
...
- **unadjusted per protocol analysis**. In an experimental study,
    ...
    from the analysis. However stoping treatment is sheldom random and
    often related to the outcome (e.g. full recovery, no improvment,
    or rapid deterioration) so not accounting for time-dependent
    confounder will likely lead to substantial bias. In most cases, a
    proper analysis require specialized statistical tools [Hernán,
...
- **comparing p-values across studies**, e.g. whether one study found
    ...
    studies). Comparing estimates and confidence intervals is more
    reasonnable.

- **present multiple adjusted effect estimates from a single model in
    a single table**. This has been refered to as "the table 2
