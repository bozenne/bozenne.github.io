...
> - Confidence intervals where evaluated assuming Student's t-distributed estimates, with the standard error being the square root of the inverse of the expected information and the degrees of freedom the number of observations minus the number of parameter for the modeling the mean.
...
not matter when interpreting the results) and can thus be omitted. There are exceptions:
...
- **Logistic model**: typically odds & odds ratio or, using
  ...
  width="30px"}) odds ratio are not collapsible: [adjusting or not may
...
- **Kruskall Wallis test**: since [its test statistic can be deduced from
  the pairwise Wilcoxon
...
- **Cox model**: hazard ratio or, using
  ...
  timepoint). <br>
  (![](https://bozenne.github.io/img/warning.jpg){:height="30px"
  width="30px"}) interpeting hazard ratios in a causal way is
  generally not straightforward and evaluating survival probabilities
  instead is [the recommended
