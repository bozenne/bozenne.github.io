...
- **software citation**: identify and giving credit to the authors of the
  software, which require a proper reference (instead of providing a
  softwar version). All R packages hosted on CRAN have a DOI and they
  generally have a CITATION file indicating how they should be
  reference in articles. These can be accessed in R by running ```r
  citation() ``` and ```R.version.string``` for referencing the R
  software, and ```r citation("lava")``` for referencing a specific R
  package (here lava).
...
- **software reproducibility**: facilitate reproducing the results or
  ...
  the version of the operating system, software, all related packages
  (output of ```r sessionInfo() ```). This is a lot of information and
...
R Core Team (2024). R version 4.3.3 (2024-02-29): A Language and Environment for Statistical
