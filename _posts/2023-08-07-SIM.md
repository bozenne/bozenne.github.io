...
adhere to the treatment. Instrumental variable methods typically
estimate treatment effect amongst compliers. This illustrates that
implications of using a specific analytical strategy (here a mixed
model on censored data) may not be obvious, so the population and the
setting with respect to which the parameter of interest is defined
should be describe whenever they differ from, respectively, all
patients and the real world.
